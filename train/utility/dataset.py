...
class AddNoise(object):
    ...
    def __init__(self, sigma, pad_noise_map=False):
        self.sigma_ratio = sigma / 255.
        self.pad_noise_mask = pad_noise_map

    def __call__(self, img):
        noise = np.random.randn(*img.shape) * self.sigma_ratio
        # print(img.sum(), noise.sum())
        noisy_img = img + noise
        if self.pad_noise_mask:
            noise_level_map = np.ones_like(img, dtype=noise.dtype) * self.sigma_ratio
            out = np.concatenate([noisy_img[None], noise_level_map[None]], axis=0)
            return out
        return noisy_img
...
class AddNoiseBlindV2(object):
    ...
    def __init__(self, min_sigma, max_sigma, pad_noise_map=False):
        self.pad_noise_map = pad_noise_map
    ...
    def __call__(self, img):
        ...
        noisy_img = img + noise
        if self.pad_noise_map:
            noise_level_map = np.ones_like(img, dtype=noise.dtype) * sigma
            out = np.concatenate([noisy_img[None], noise_level_map[None]], axis=0)
            return out
        return noisy_img
...
class HSI2Tensor(object):
    ...
    def __init__(self, use_2dconv, pad_noise_map=False):
        self.use_2dconv = use_2dconv
        self.pad_noise_map = pad_noise_map

    def __call__(self, hsi):
        if self.use_2dconv or self.pad_noise_map:
...
class LoadMatHSI2(object):
    def __init__(self, input_key, gt_key, noise_level_key='sigma', transform=None, pad_noise_map=False):
        self.gt_key = gt_key
        self.input_key = input_key
    ...
    def __call__(self, mat):
        ...
        gt = torch.from_numpy(gt).float()
        if self.pad_noise_map:
            sigma = (mat[self.noise_level_key][:] / 255.).astype('float32')
            noisy_level_map = torch.ones_like(input) * sigma
            input = torch.cat([input, noisy_level_map], dim=0)
        return input, gt
